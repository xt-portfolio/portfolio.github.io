...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
  anchor.addEventListener('click', function (e) {
    ...
    if (this.pathname === window.location.pathname) {
      // 这里不阻止默认行为，使用浏览器原生锚点跳转
      // 这样点击时会直接跳转到对应位置，不显示滚动动画
      return true;
    }
  });
});
...
ScrollReveal().reveal(".enlarge-motion,.more-projectCards .project-card", { ...staggeringOption ,scale:0.93,distance: "0px",duration: 900,interval:100});
